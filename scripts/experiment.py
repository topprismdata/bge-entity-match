...
功能：
  1. 运行多组配置实验（不同 TopK、是否城市分治、不同清洗策略）
...
def run_exp(query_df, cand_df, config, args):
    ...
    if args.gt_id:
        ...
        t1 = t3 = t10 = 0
        n = len(q_valid)
        rank_data = {r['q_idx']: r for r in rows}
...
def main():
    ...
    for i, cfg in enumerate(configs):
        ...
        print(f"  [{i+1}/{len(configs)}] TopK={cfg['topk']:>3} {city_str} → {acc} ({r['elapsed_s']}s)", flush=True)
    ...
    for r in sorted(results, key=lambda x: -(x.get('top1') or 0)):
        ...
        md.append(f"| {r['config']['topk']:>3} | {city_str} | {t1} | {t3} | {t10} | {r['elapsed_s']}s |")

    md += ["", f"**最优配置**: TopK={best['config']['topk']}, 城市分治={best['config']['use_city']}"]
